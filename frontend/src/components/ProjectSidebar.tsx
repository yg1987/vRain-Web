...
export default function ProjectSidebar({
  ...
  return (
    <div>
      <button
        onClick={() => navigate("/")}
        className="mb-4 flex items-center gap-1 text-xs text-ink/65 hover:text-ink/90 transition-colors"
      >
      ...
      <h3 className="mb-4 text-sm font-semibold text-ink/90">项目</h3>
      ...
      <div className="mb-4 rounded-lg border border-ink/10 bg-white/40 p-3 space-y-2">
        ...
        {/* 字体列表 */}
        <div>
          <div className="text-xs font-semibold text-ink/85 mb-1">字体 ({fonts.length})</div>
          {fonts.length > 0 ? (
            <div className="space-y-1">
              {fonts.map((font, i) => (
                <div key={i} className="flex items-center gap-1.5 text-xs text-ink/75">
                  <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-ink/10 text-[9px] font-bold text-ink/75">
                    {i + 1}
                  </span>
            ...
            <div className="text-xs text-ink/75">未加载字体</div>
          )}
        </div>
      </div>
    </div>
